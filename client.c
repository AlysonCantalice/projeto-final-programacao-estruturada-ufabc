...
// • bignumber.h um arquivo de header de C contendo toda a interface pública do seu tipo
// BigNumber.
// • bignumber.c um arquivo de C contendo a implementação da sua interface pública.
// • client.c um arquivo de C contendo a função main() e que é responsável por usar a sua
// biblioteca bignumber.h para resolver o problema do projeto.
...
int main(){
    ...
    while (1) {
        ...
        // to finish the program using the terminal when doesn't have input
        if (first_line[0] == '\0'){
            free(first_line);
            break;
        }
